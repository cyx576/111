...
def train(model, kgs, args, out_folder):
    ...
    for i in range(1, args.max_epoch + 1):
        ...
        for _ in range(relation_triple_steps):
            ...
            # ------------------ 【核心修复】: 梯度钳制 ------------------
            # 确保权重更新的幅度不会爆炸，防止 NAN 出现
            # max_norm=0.5 是我们为了稳定训练而收紧的值
...
if __name__ == '__main__':
    ...
    best_model_path = out_folder + 'model_best.pkl'
    if os.path.exists(best_model_path):
        model.load_state_dict(torch.load(best_model_path))
        mrr = test(args.max_epoch + 1, model, kgs, args)
    else:
        print(f"Warning: Best model file not found at {best_model_path}. Skipping final test.")
