...
def test(epoch, model, kgs, args, save=True):
    ...
    with torch.no_grad():
        ...
        embeds1, embeds2, e_r1, e_r2, e_i1, e_i2, e_a1, e_a2 = model.predict(e1, e2, at_mask1, im_mask1, at_mask2,
                                                                             im_mask2)
        
        rest_12, hits1_12, mr_12, all_mrr_12 = greedy_alignment(embeds1, embeds2, args.top_k, args.test_threads_num,
                                                                metric=args.eval_metric, normalize=args.eval_norm,
...
def train(model, kgs, args, out_folder):
    ...
    align_criterion = ContrastiveLoss(args.margin).cuda()
    ...
    for i in range(1, args.max_epoch + 1):
        ...
        for _ in range(relation_triple_steps):
            ...
            rs, ats, ims, score, orth_loss, mmd_loss = model.predict(train_e1, train_e2, at_mask1, im_mask1, at_mask2,
                                                                     im_mask2, 'train')
            align_loss = align_criterion(score, label_) + align_criterion(rs, label_) + align_criterion(ats,
                                                                                                        label_) + align_criterion(
                ims, label_)
            # loss = r_loss + align_loss + orth_loss + mmd_loss
            loss = r_loss + align_loss + orth_loss + mmd_loss * args.mmd_factor
            
            # # 🚨 【DEBUG 步骤】：打印原始损失值
            # try:
            #     print(
            #         f"r_loss: {r_loss.item():.8f}, align_loss: {align_loss.item():.8f}, orth_loss: {orth_loss.item():.8f}, mmd_loss: {mmd_loss.item():.8f}")
            # except Exception as e:
            #     print(f"r_loss: {r_loss}, align_loss: {align_loss}, orth_loss: {orth_loss}, mmd_loss: {mmd_loss}")
            #     print(f"打印失败，可能存在 NaN/Inf。")
            
            loss.backward()
            # 【核心修复】: 梯度钳制 (防止权重更新幅度在任何阶段爆炸)
        ...
        print(
            'loss: {:.8f}, relation loss: {:.8f}, align loss:{:.8f}, orth loss: {:.8f}, mmd loss: {:.8f}, time: {:.4f}s'.format(
                epoch_loss, epoch_rloss, epoch_celoss, epoch_orth_loss, epoch_mmd_loss, end - start))
        ...
        if args.iterative and i >= args.start_valid and i % args.eval_freq == 0:
            ...
            kgs.relation_triples_list1, kgs.relation_triples_list2 = kgs.add_sup_relation_triples(sup_triples1_set,
                                                                                                  sup_triples2_set)
